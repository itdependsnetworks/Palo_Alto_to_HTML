...
class AjaxTableEditor
{
	...
	function setDefaults()
	{
		...
		$this->start = 0;
		$this->displayNum = isset($_SESSION[$this->varPrefix.'displayNum']) ? $_SESSION[$this->varPrefix.'displayNum'] : 50;
		$this->ascOrDesc = 'asc';
...
/*
	function exportInfo()
	{
		...
		$query = $this->selectClause.' '.$this->joinClause.' '.$this->whereClause.' order by '.$this->orderByColumn.' '.$this->getAscOrDesc().' '.$this->extraOrderByInfo;
		$result = $this->doQuery($query);
...
#				if(stristr($this->permissions,'H'))
#				{
					$csvInfo .= "<tr><td>".implode('</td><td>', $csvRow)."</td></tr>";
#				}
#				else
#				}
#					$csvInfo .= implode(',', $csvRow).$csvLb;
#				}
	...
	function exportInfo()
	{
		...
		#		$exportInfo .= implode($this->exportDelimiter,$exportRow).$this->exportLineBreak;
				$exportInfo .= "<tr><td>".implode('</td><td>', $exportRow)."</td></tr>";;
			}
			return $exportInfo;
		}
	}
